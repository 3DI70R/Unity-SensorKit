...
public static class PhysicsSensorUtils
{
    ...
    public static void HighlightMeshVertices(RaycastHit hit)
    {
        ...
        if (meshCollider != null)
        {
            ...
            var v0 = meshCollider.transform.TransformPoint(sharedVertices[sharedIndices[triangleStart]]) + normalOffset;
            var v1 = meshCollider.transform.TransformPoint(sharedVertices[sharedIndices[triangleStart + 1]]) + normalOffset;
            var v2 = meshCollider.transform.TransformPoint(sharedVertices[sharedIndices[triangleStart + 2]]) + normalOffset;
    ...
    public static void DrawCapsuleGizmo(Vector3 position, Quaternion rotation, Vector3 scale, float width, float radius)
    {
        Vector3 p1;
        Vector3 p2;
